...
export const DashboardTeamsList = () => {
  ...
  const columns: ColumnDef<TeamInformationBrief>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className="text-sm text-muted-foreground line-clamp-1 max-w-[300px]" title={str}>
            {str}
          </span>
        )
      },
  ...
  return (
    <div className="flex flex-col w-full h-full min-h-0">
      ...
      <AllTeamsListDialog open={allTeamsDialogOpen} openChanged={setAllTeamsDialogOpen} />
      ...
      <div className="flex items-center py-4 mt-2">
        <Input
          ...
          className="max-w-md"
          onChange={(event) => table.getColumn('friendlyName')?.setFilterValue(event.target.value)}
        />

        <div className="flex-grow-1"></div>
        <Button
          onClick={() => {
            setCreateNewTeamDialogOpen((open) => !open)
          }}
        >
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => {
                    navigate(`/org/teams/${row.original.pk}`)
                  }}
      ...
      <div className="flex flex-col items-center mt-5">
        ...
        <div
          ...
          onClick={() => setAllTeamsDialogOpen(true)}
        >
  ...
  const handleFormSubmit = () => {
    ...
    fetch(submitURL, {
      ...
      .then(async (res) => {
        ...
        if (res.status === 201) {
          toast.success(
            `New ${teamType.toLowerCase()} team for ${teamName} (${teamSeason} ${teamYear}) successfully created!`
          )
    ...
    <Dialog open={props.open} onOpenChange={props.openChanged}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            We're glad you're at this page to create a new team and to help the {ORGANIZATION_NAME}{' '}
            expand!
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4">
          <Alert>
            ...
            <AlertDescription>
              <span>
                If you're leading a project, an effective team name would be like US News or Amazon
                Kuiper. If you're an internal team, an example team name would be Social Media.
                Please <b>ensure appropriate capitalization</b> and <b>do not append</b> terms like
                Fall 2024, etc.
              </span>
            </AlertDescription>
          </Alert>
          ...
          <div className="grid gap-3">
            ...
            <Textarea
              maxLength={200}
              placeholder="Ex. Developing an MLOps pipeline for Brain Tumor Segmentation Models"
              required
          ...
          <div className="grid gap-3">
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  <SelectItem value="CORPORATE">
                    Corporate (All Other Teams, Ex. Sponsorhip)
                  </SelectItem>
                </SelectGroup>
              </SelectContent>
            </Select>
            ...
            <div
              className={`flex items-center gap-3 ${teamType == 'PROJECT' || teamType == 'BOOTCAMP' ? '' : 'hidden'}`}
            >
              <Checkbox
                checked={projectLeadConfirmed}
                onCheckedChange={(checked) => setProjectLeadConfirmed(checked != false)}
                id="projectlead_confirm"
              />
              <Label htmlFor="projectlead_confirm">
                {teamType == 'PROJECT'
                  ? 'I confirm that I will be the Project Lead for this team'
                  : 'I confirm that I will be the Bootcamp Director for this team'}
              </Label>
            </div>
            ...
            <div className={`grid gap-3 ${teamType == 'CORPORATE' ? '' : 'hidden'}`}>
              ...
              <Input
                placeholder="Ex. Team Lead, Director, etc."
                required
                value={teamRole}
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              isLoading || teamName.trim().length < 3 || !teamSeason || !teamType || !isRoleValid()
            }
          >
...
const AllTeamsListDialogContent = ({
  open,
  openChanged,
}: {
  open: boolean
    ...
    <Dialog open={open} onOpenChange={openChanged}>
      <DialogContent className="sm:max-w-2xl h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>Browse all active teams in the organization here!</DialogDescription>
        </DialogHeader>
        ...
        <div ref={setParentRef} className="flex-grow w-full overflow-y-auto border rounded-md">
          <div
            style={{
        ...
        <DialogFooter className="sm:justify-between items-center text-xs text-muted-foreground">
          ...
          <Button variant="outline" size="sm" onClick={() => openChanged(false)}>
            Close
          </Button>
        </DialogFooter>
      </DialogContent>
