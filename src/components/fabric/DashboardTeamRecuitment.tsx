...
export const DashboardTeamRecruitment = () => {
  ...
  const [roleSpecQuestions, setRoleSpecQuestions] = React.useState<{
    [key: string]: { [key: string]: string[] }
  }>({})

  const [applications, setApplications] = React.useState<KanbanApplicationCard[]>([])
  const [selectedApplication, setSelectedApplication] =
    React.useState<KanbanApplicationCard | null>(null)
  const [selectedApplicationDetails, setSelectedApplicationDetails] =
    React.useState<KanbanApplicationDetails | null>(null)
  ...
  const [actionDialog, setActionDialog] = React.useState<
    'reject' | 'interview' | 'potential' | 'hired' | null
  >(null)
  ...
  function handleStarRating(stars: number) {
    ...
    setApplications((prev) =>
      prev.map((app) => (app.id === selectedApplication.id ? { ...app, stars } : app))
    )

    fetch(
      `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/applications/${params.teamId}/${selectedApplication.id}/feedback`,
      {
        method: 'PUT',
      ...
      .catch((err) => {
        ...
        setSelectedApplication((prev) =>
          prev ? { ...prev, stars: selectedApplication.stars } : null
        )
  ...
  function executeStageUpdate(applicationId: string, newStage: string, extraData?: any) {
    ...
    setApplications((apps) =>
      apps.map((a) => (a.id === applicationId ? { ...a, column: newStage, stage: newStage } : a))
    )

    fetch(
      `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/applications/${params.teamId}/${applicationId}/stage`,
      {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (app && app.stage !== over.id) {
      // over.id corresponds to column id if dropped on column
  ...
  function fetchApplications() {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/applications/${params.teamId}`, {
      credentials: 'include',
    })
      .then(async (response) => {
  ...
  React.useEffect(() => {
    if (selectedApplication?.applicantId) {
      ...
      fetch(
        `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/applications/${params.teamId}/${selectedApplication.applicantId}/resume`
      )
        .then(async (res) => {
          if (!res.ok) {
            const errorData = await res.json()
      ...
      fetch(
        `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/applications/${params.teamId}/${selectedApplication.applicantId}/otherapps`
      )
        .then(async (res) => {
          if (!res.ok) {
            const errorData = await res.json()
      ...
      // Fetch Full Details
      fetch(
        `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/applications/${params.teamId}/${selectedApplication.id}/info`
      )
        .then(async (res) => {
          if (!res.ok) {
            const errorData = await res.json()
  ...
  React.useEffect(() => {
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/teams/${params.teamId}`)
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(teamlistResponse.message || 'Failed to fetch')

        setTeamInfo(teamlistResponse.team)
        setSubTeams(teamlistResponse.subteams)

        for (const subteam of teamlistResponse.subteams) {
          fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/config/${subteam.pk}`).then(
            async (response) => {
              if (response.ok) {
                const config: SubteamATSConfig = await response.json()
                const mappedRoles = config.roles.map(
                  (role, index) => ({ id: index.toString(), text: role }) as Tag
                )
                setRoles((role) => ({ ...role, [subteam.pk]: mappedRoles }))
                setTagIndex((tagIndex) => ({ ...tagIndex, [subteam.pk]: null }))
                setRecruitmentEnabled((enabled) => ({
                  ...enabled,
                  [subteam.pk]: config.isRecruiting,
                }))
                setRoleSpecQuestions((rsq) => ({
                  ...rsq,
                  [subteam.pk]: config.roleSpecificQuestions,
                }))
  ...
  function handleSettingsUpdate(subteamPk: string) {
    ...
    const questions = enabledRoles.reduce((acc: { [key: string]: string[] }, role) => {
  ...
  const currentNavIndex = selectedApplication
    ? reviewableApps.findIndex((a) => a.id === selectedApplication.id)
    : -1
      ...
      <div className="flex items-center">
        <div className="flex flex-col flex-grow-1">
          ...
          <h4 className="text-xl text-muted-foreground">
            {teamInfo?.attributes.friendlyName}{' '}
            {`${teamInfo?.attributes.seasonType} ${teamInfo?.attributes.seasonYear}`}
          </h4>
        </div>
      </div>
        ...
        <div className="mt-2 flex-grow flex flex-col min-h-0">
                ...
                return (
                  <div className="w-[250px] shrink-0">
                    <KanbanBoard id={column.id} className="w-full ring-inset">
                      <KanbanHeader>
                        {column.name} ({applications.filter((a) => a.stage === column.id).length})
                      </KanbanHeader>
          ...
          <TabsContent value="settings">
            <Accordion type="single" collapsible>
              {subTeams
                .filter((subteam) => !subteam.attributes.flaggedForDeletion)
                .map((subteam) => {
                  return (
                      ...
                      <AccordionContent>
                        <div className="flex flex-col">
                          ...
                          <div className="flex items-center space-x-2 mt-2">
                            <Switch
                              checked={recruitmentEnabled[subteam.pk]}
                            ...
                            <Label htmlFor="airplane-mode">
                              Enable Recruiting for the{' '}
                              {subteam.attributes.friendlyName.toUpperCase()} subteam
                            </Label>
                          </div>
                          ...
                          <p>
                            Please note that the roles you define below are only for the{' '}
                            <b>{subteam.attributes.friendlyName.toUpperCase()}</b> subteam. Here's a
                            quick guide of sample roles, just in case you're stuck!
                          </p>
                          <ul className="list-disc pl-8 pt-2">
                            <li>
                              Leadership Subteams may recruit <b>Tech Leads</b>
                            </li>
                            <li>
                              Engineering Subteams may recruit{' '}
                              <b>Frontend Engineers, Full Stack Engineers, etc.</b>
                            </li>
                            <li>
                              Bootcamp's Students Subteam would only recruit for the role of{' '}
                              <b>Bootcamp Student</b>
                            </li>
                            <li>
                              Social Media's Creative Subteam may recruit for{' '}
                              <b>Poster Designers</b>
                            </li>
                          </ul>
                          ...
                          <TagInput
                            ...
                            setTags={(value: React.SetStateAction<Tag[]>) => {
                              setRoles((roles) => ({
                                ...roles,
                                [subteam.pk]:
                                  typeof value == 'function' ? value(roles[subteam.pk]) : value,
                              }))
                            }}
                            styleClasses={{
                            ...
                            setActiveTagIndex={function (
                              ...
                              setTagIndex((tagIndex) => ({
                                ...tagIndex,
                                [subteam.pk]:
                                  typeof value == 'function' ? value(tagIndex[subteam.pk]) : value,
                              }))
                            }}
                          />
                          ...
                          {roles[subteam.pk] && roles[subteam.pk].length > 0 ? (
                            ...
                            <p>
                              Please Create roles in the previous section to enable Role Specific
                              Questions
                            </p>
                          )}
                        </div>
                        ...
                        <Button
                          disabled={isLoading}
                          onClick={() => handleSettingsUpdate(subteam.pk)}
                          className="mt-5"
                        >
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    disabled={
                      currentNavIndex === -1 || currentNavIndex >= reviewableApps.length - 1
                    }
                  >
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className="flex-1"
                    onClick={() =>
                      selectedApplicationDetails?.email &&
                      (window.location.href = `mailto:${selectedApplicationDetails.email}`)
                    }
                  ...
                  <Button
                    ...
                    className="flex-1"
                    onClick={() =>
                      selectedApplicationDetails?.profile?.linkedinUrl &&
                      window.open(selectedApplicationDetails.profile.linkedinUrl, '_blank')
                    }
                  ...
                  <Button
                    ...
                    className="flex-1"
                    onClick={() =>
                      selectedApplicationDetails?.profile?.githubUrl &&
                      window.open(selectedApplicationDetails.profile.githubUrl, '_blank')
                    }
                ...
                {selectedApplicationDetails?.appDevInternalPk && (
                  <Alert className="bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/20 [&>svg]:text-amber-600 dark:[&>svg]:text-amber-400">
                    ...
                    <AlertDescription>
                      <span>
                        {selectedApplicationDetails?.name.split(' ')[0]} is already in App Dev. To
                        view more about their team history and their internal profile, please{' '}
                        <NavLink
                          to={`/org/people/${selectedApplicationDetails?.appDevInternalPk}`}
                ...
                {/* Subteam Preferences */}
                <div>
                  <h4 className="text-md text-muted-foreground mb-2">
                ...
                <div>
                  ...
                  <div className="bg-muted/50 p-3 rounded">
                    <div className="flex items-start">
                      <p className="text-sm font-medium mb-1 flex-grow-1">
                        Interview and Intial Feedback
                      </p>
                      <Button
                        onClick={() => {
                    ...
                    <div className="text-sm text-muted-foreground">
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          h6: ({ node, ...props }) => (
                            <h6 className="font-semibold mt-2" {...props} />
                          ),
                          p: ({ node, ...props }) => (
                  ...
                  {selectedApplicationDetails?.profile?.instagramFollow && (
                    <div className="bg-muted/50 p-3 rounded">
                      <p className="text-sm font-medium mb-1">
                        Do you follow App Dev on Instagram?
                      </p>
                ...
                {selectedApplicationDetails?.profile?.whyAppDev && (
                  <div className="bg-muted/50 p-3 rounded">
                    <p className="text-sm font-medium mb-1">
                      Why are you interested in joining App Dev?
                    </p>
                    <p className="text-sm text-muted-foreground whitespace-pre-wrap">
                      {selectedApplicationDetails.profile.whyAppDev}
                    </p>
                ...
                {selectedApplicationDetails?.responses &&
                  Object.keys(selectedApplicationDetails.responses).length > 0 && (
                    <div>
                      <h4 className="text-md text-muted-foreground mb-4">
                        Role Specific Responses
                      </h4>
                      <div className="space-y-3">
                        {Object.entries(selectedApplicationDetails.responses).map(
                          ([question, answer]) => (
            ...
            <div className="h-full flex flex-col gap-6 pt-2">
                ...
                {stages.map((stage) => (
                  <Button
                    ...
                    disabled={
                      !selectedApplication ||
                      selectedApplication.stage === stage.id ||
                      !validateTransition(selectedApplication.stage, stage.id)
                    }
                  >
                    {stage.name}
                  </Button>
              ...
              <div className="flex gap-2 max-w-full overflow-x-auto">
                {otherApplications.map((app) => (
                  <div
                    key={app.id}
                    className={`flex gap-6 p-3 rounded-md border text-sm w-[200px] shrink-0 ${app.id === selectedApplication?.id ? 'bg-primary/10 border-primary' : 'bg-background hover:bg-muted/50'}`}
                  >
                    ...
                    <Badge className={`${STAGE_STYLES[app.stage]} capitalize`} variant="secondary">
                      {app.stage}
                    </Badge>
                  </div>
                ))}
              ...
              <p className="text-lg text-muted-foreground leading-0">
                {selectedApplication?.name.split(' ')[0]}'s Resume
              </p>
              <iframe
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Rejecting an applicant sends a formal email to them. If you're sure they don't meet
              App Dev expectations or if they can't be accomodated in your team, confirm this
              action.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={cancelStageUpdate}>
              Cancel
            </Button>
            <Button
              variant="destructive"
              onClick={() =>
                pendingTransition &&
                executeStageUpdate(pendingTransition.applicationId, pendingTransition.newStage)
              }
            >
              Reject{' '}
              {
                applications
                  .find((a) => a.id === pendingTransition?.applicationId)
                  ?.name.split(' ')[0]
              }
            </Button>
          </DialogFooter>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Yay! We're happy that{' '}
              {
                applications
                  .find((a) => a.id === pendingTransition?.applicationId)
                  ?.name.split(' ')[0]
              }{' '}
              meets our initial expectations. Please provide a Google Calendar link for your
              availability and we'll email them.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center text-xs text-muted-foreground">
                ...
                <span
                  className={cn(
                    interviewGuidelines.length < 50 || interviewGuidelines.length > 500
                      ? 'text-destructive'
                      : 'text-green-600'
                  )}
                >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              an email to inform they've passed the interview and are on hold considering space
              availability.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              meets App Dev's talent bar. We'll send them a unique onboarding link to officially
              join the team.
            </DialogDescription>
          </DialogHeader>
          <div>
            <Label className="mb-3">
              Please choose the role that you'd like to recruit them for:
            </Label>
              ...
              <SelectContent>
                {applications
                  ...
                  ?.rolePreferences?.map((pref) => (
                    <SelectItem key={pref.role} value={pref.role}>
                      {pref.role}
                    </SelectItem>
                  ))}
              </SelectContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                executeStageUpdate(pendingTransition.applicationId, pendingTransition.newStage, {
                  hiredRole,
                  hiredSubteamPk: targetSubteamPk,
                })
              }}
            >
              Onboard{' '}
      ...
      <Dialog open={feedbackDialogOpen} onOpenChange={(open) => setFeedbackDialogOpen(open)}>
        <DialogContent className="min-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              You're feedback means a lot towards helping make informed hiring decisions. Please
