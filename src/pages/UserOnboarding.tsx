...
export const UserOnboarding = () => {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/invites/${params.onboardId}`, {
      ...
      .catch(() => {
        ...
        toast.error('Onboarding Failed!', {
          description:
            "Please contact your team's leadership for assistance. Futher information can be found in your invite email.",
        })
      })
  }
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      {/* Add Stages Here */}
      <div
        style={{ height: 'calc(100% - calc(var(--spacing) * 12))' }}
                ...
                <SidebarGroupContent style={{}}>
                  <SidebarMenu>
                    {ONBOARDING_FLOWLIST.map((el, index) => (
                      <SidebarMenuItem>
                        <SidebarMenuButton asChild isActive={location.pathname.endsWith(el.path)}>
                          <Link
                            onClick={() => {
          ...
          <div className="flex flex-col items-center justify-center h-full flex-grow-1">
            <Routes>
              ...
              <Route
                ...
                element={
                  <CreatePasswordStage
                    defaultPassword={createdPasswordRef.current}
                    {...passwordStageProps}
                  />
                }
              />
              <Route
                path="/legal"
                element={
                  <ProprietaryInformationStage
                    defaultSigned={ipAgreementComplete.current}
                    stepComplete={handleIPAgreementComplete}
                  />
                }
              />
              <Route
                path="/slack"
                element={
                  <SlackJoinStage defaultVerified={slackJoinComplete.current} {...slackJoinProps} />
                }
              />
              <Route
                path="/identity"
                element={
                  <PersonalInfoStage
                    onboardId={params.onboardId}
                    defaultData={personalInfoRef.current}
                    {...personalInfoProps}
                  />
                }
              />
                  ...
                  <CompleteSetupStage
                    ...
                    stages={[
                      { name: 'Password Creation', status: createdPasswordRef.current.length >= 8 },
                      {
                        ...
                        status: personalInfoRef.current != undefined,
                      },
                      {
                        name: 'Intellectual Property Agreement',
                        status: ipAgreementComplete.current,
                      },
  ...
  return (
    <div className="flex flex-col h-full w-full justify-center items-center p-12">
      ...
      <CardDescription className="text-center">
        Please complete any incomplete items and once you're done, smash the button! 💥
      </CardDescription>
                ...
                <TableCell>
                  {stage.status ? (
                    <span className="flex gap-1 items-center text-green-500">
                      <CheckCircle2Icon size="16" /> Complete
                    </span>
                  ) : (
                    <span className="flex gap-1 items-center text-red-400">
                      <XCircleIcon size="16" /> Incomplete
                    </span>
                  )}
                </TableCell>
              </TableRow>
...
const PersonalInfoStage = (props: PersonalInfoStageProps) => {
  ...
  const [selectedMajor, setSelectedMajor] = React.useState<UMDApiMajorListResponse | undefined>(
    props.defaultData?.major
  )
  ...
  const [expectedGraduation, setExpectedGraduation] = React.useState(
    props.defaultData?.expectedGrad ?? ''
  )
    ...
    if (file) {
      ...
      if (!isValidSignature) {
        toast.error('Invalid file content', {
          description: 'The file content does not match its extension.',
        })
        return
      }
  ...
  async function processAndUploadAvatar() {
    ...
    try {
      ...
      const res = await fetch(
        `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/people/avatar/upload-url?inviteId=${props.onboardId}&fileName=${encodeURIComponent(uploadFile.name)}&contentType=${encodeURIComponent(uploadFile.type)}`
      )
      if (!res.ok) {
  ...
  return (
    <div className="flex flex-col h-full w-full items-center justify-center p-12">
      <Avatar
        ...
        className="size-32 rounded-full cursor-pointer"
      >
      ...
      >
        <DialogContent className="max-w-xl">
          ...
          <DialogFooter className="mt-4">
            <Button variant="outline" onClick={() => setIsCroppingOpen(false)}>
              Cancel
            </Button>
      ...
      <div className={'grid gap-2 w-lg mt-5'}>
        ...
        <Popover open={majorListOpen} onOpenChange={setMajorListOpen}>
          <PopoverTrigger asChild>
            ...
            >
              {selectedMajor ? selectedMajor.name : 'No Major Chosen'}
                  ...
                  {majors.map((framework) => (
                    ...
                    >
                      ...
                      <Check
                        className={cn(
                          'ml-auto',
                          selectedMajor?.major_id === framework.major_id
                            ? 'opacity-100'
                            : 'opacity-0'
                        )}
                      />
...
const SlackJoinStage = (props: SlackJoinStageProps) => {
  ...
  const verifyJoinStatus = () => {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/tools/verifyslack`, {
      ...
      .then(async (res) => {
        ...
        if (status !== true) throw new Error('Not Joined!')
      ...
      .catch(() => {
        ...
        toast.error('Verification Failed!', {
          description:
            "We couldn't verify that you're in the App Dev Slack. Please make sure that you used the correct email address and invite link as mentioned in the instructions.",
        })
      })
  }
  ...
  return (
    <div className="flex flex-col h-full w-full justify-center items-center p-12">
      ...
      <CardDescription className="text-center">
        That's where all the fun stuff cooks 🥘
      </CardDescription>
...
const ProprietaryInformationStage = (props: ProprietaryInformationStageProps) => {
  ...
  return (
    <div className="flex flex-col h-full w-full justify-center items-center p-12">
      ...
      <CardDescription className="text-center">
        This legal agreement helps us transfer the technology that you've helped develop directly to
      ...
      <div className="flex w-full items-start gap-3 mt-5">
        ...
        <div className="grid flex-grow-1">
          ...
          <p className="text-muted-foreground text-sm">
            By clicking this checkbox, you agree to have signed the aforementioned Intellectual
            Property Agreement with App Dev Club LLC.
          </p>
        </div>

        <Button
          onClick={() => {
            props.stepComplete(agreementSigned)
          }}
    ...
    <div className="flex flex-col h-full w-full justify-center items-center p-12">
      ...
      <div className="flex flex-col items-center gap-4 mt-5 w-full">
        <Alert variant="default" className="w-lg">
          ...
          <AlertDescription>
            Hello {props.name}! You will need an App Dev Club account to access internal
            repositories, resources and people portals. Your login email will be {props.email}.
            Please confirm that you'll be joining the App Dev's {props.teamName} team as a{' '}
            {props.role}.
          </AlertDescription>
        </Alert>
              ...
              <div className="space-y-3">
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={cn(
                        'text-xs font-bold',
                        getStrengthColor(strengthScore).replace('bg-', 'text-')
                      )}
                    >
                ...
                <div className="space-y-1">
                  ...
                  {passwordRequirements
                    .filter((req) => req.name !== 'Passwords match')
                    .map((req, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <span
                          className={cn(
                            'text-xs',
                            req.status ? 'text-foreground' : 'text-muted-foreground'
                          )}
                        >
                          {req.name}
                        </span>
      ...
      {/* Next Step Stuff */}
      <Button
        disabled={password.length < 12 || password != confirmPassword || strengthScore < 2}
        className="mt-8"
        onClick={() => {
          props.stepComplete(password)
        }}
      >
