...
export const DashboardTeamInfo = () => {
  ...
  const [removalPendingInfo, setRemovalPendingInfo] = React.useState<{
  ...
  function syncBindles() {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/teams/${params.teamId}/syncbindles`, {
      ...
      .then(async (response) => {
        ...
        toast.success(
          `Shared Permissions Synced for the ${teamInfo?.attributes.friendlyName} team!`
        )
      })
      .catch((e) => {
  ...
  function saveRootTeamSettings(settings: {
    [clientName: string]: { [settingKey: string]: boolean }
  }) {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/teams/${params.teamId}`, {
      ...
      .then(async (attrResponse) => {
        if (!attrResponse.ok) {
          ...
          throw new Error(
            responseData.message || `Attributes Update Failed: HTTP ${attrResponse.status}`
          )
        }
  ...
  function generateAWSMagicLink() {
    ...
    setSyncDialogDescription(
      "Please wait while a new AWS Console session is created. You'll be redirected automatically."
    )
    setSyncDialogOpen(true)
  ...
  return (
    <div className="flex flex-col m-2">
      ...
      <AddTeamMembersDialog
        open={addMembersOpen}
        openChanged={setAddMembersOpen}
        teamPk={teamInfo?.pk}
        subteams={subTeams.filter((t) => !t.attributes.flaggedForDeletion)}
      />
      <ProgressUpdateDialog
        ...
        progressPercent={syncDialogProgress}
      />
      <SubteamsInfoDialog
        open={subteamsOpen}
        openChanged={setSubteamsOpen}
        subteams={subTeams}
        onRefresh={refreshTeamInfo}
        parentTeamId={teamInfo?.pk}
      />
      <TeamSettingsDialog
        open={teamSettingsOpen}
        openChanged={setTeamSettingsOpen}
        teamInfo={teamInfo}
        settingDefinitions={settingDefinitions}
        isSaving={isSavingSettings}
        onSave={saveRootTeamSettings}
      ...
      <div className="flex items-center">
        <div className="flex flex-col flex-grow-1">
          <h1 className="scroll-m-20 text-4xl font-extrabold tracking-tight text-balance">
            ...
            <span className="text-2xl ml-1 text-muted-foreground">{`${teamInfo?.attributes.seasonType} ${teamInfo?.attributes.seasonYear}`}</span>
          </h1>
      ...
      <Tabs
        key={teamInfo?.pk ?? 'loading'}
        className="mt-5"
        defaultValue={
          teamInfo?.attributes.seasonType === 'ROLLING'
            ? (subTeams?.filter((st) => !st.attributes.flaggedForDeletion)[0]?.name ?? 'owner')
            : 'owner'
        }
      >
        ...
        <TabsList>
          ...
          {subTeams
            ?.filter((st) => !st.attributes.flaggedForDeletion)
            .map((subteam) => {
              let tabName = subteam.attributes.friendlyName
              if (tabName.endsWith('Engr')) tabName = 'Engineering Team'
              else if (tabName.endsWith('Lead')) tabName = 'Project Leadership'
            ...
            .map((subteam) => (
              <TabsContent value={subteam.name}>
                <UserInformationTable
                  ...
                  teamPk={subteam.pk}
                  onRemove={(user) =>
                    handleRemoveMember(user, subteam.pk, subteam.attributes.friendlyName)
                  }
                />
              </TabsContent>
            ))}
...
const AddTeamMembersDialog = (props: {
  teamPk?: string
  subteams: TeamInfo[]
  open: boolean
  ...
  const handleMemberAdd = async () => {
    ...
    if (currentTab == 'existing') {
      if (!selectedSubTeam || !selectedExistingMember) return
      ...
      fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/teams/${selectedSubTeam.pk}/addmember`, {
        ...
        .catch((err) => {
          toast.error(
            `Failed to add ${selectedExistingMember.name} to your team! Error: ${err.message}`
          )
      ...
      if (!selectedSubTeam || !props.teamPk) return
      ...
      <form>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Yay! We're happy that you're growing your team, thanks for helping the{' '}
            ...
            >
              ...
              <TabsContent className="flex flex-col gap-2" value="invite">
                <Alert>
                  ...
                  <AlertDescription>
                    By directly adding a member, you're bypassing App Dev's standard recruitment
                    procedures. Please use this feature only if you are entirely sure that this
                    person would be a great fit for the team and that they would align with App
                    Dev's culture and standards.
                  </AlertDescription>
                </Alert>
                ...
                <Input
                  ...
                  placeholder="Ex. Atheesh Thirumalairajan"
                />
            ...
            <Popover modal open={subTeamSelectionOpen} onOpenChange={setSubTeamSelectionOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" role="combobox" className="w-full justify-between">
                  {selectedSubTeam ? selectedSubTeam.name : 'Select a Permissions Group'}
  ...
  return (
    <Popover modal open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {selected ? selected.name : 'Select an Existing Member'}
          <ChevronsUpDown className="opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-full p-0">
        <Command>
          <CustomPopoverFilterBox
            isLoading={isLoading}
            onChange={(e) => getFilteredMembersList(e.target.value)}
          />
          <CommandList>
            <CommandEmpty>
              {searchValue.length < 3 ? 'Start Typing to Search...' : 'No Members Found!'}
            </CommandEmpty>
            <CommandGroup>
...
const SubteamsInfoDialog = (props: {
  ...
  const [enabledBindles, setEnabledBindles] = React.useState<{
  ...
  const sortedSubteams = React.useMemo(() => {
    return [...props.subteams].sort((a, b) =>
      a.attributes.friendlyName.localeCompare(b.attributes.friendlyName)
    )
  }, [props.subteams])

  React.useEffect(() => {
    if (props.open && !currentSubTeam) setCurrentSubTeam(sortedSubteams[0])
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.openChanged}>
      <DialogContent
        ...
        className="min-w-[60%] h-[80vh] p-0 select-none"
      >
          ...
          <SidebarGroup className="w-[30%] h-full overflow-y-auto border-r select-none">
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                ...
                {sortedSubteams.map((subteam) => (
                  <SidebarMenuItem key={subteam.pk}>
                    <SidebarMenuButton
                      ...
                      onClick={() => {
                        if (!subteam.attributes.flaggedForDeletion)
                          setCurrentSubTeam((_) => subteam)
                      }}
                    ...
                    >
                      <a>
                        <Users2Icon
                          className={subteam.attributes.flaggedForDeletion ? 'opacity-50' : ''}
                        />
                        <span
                          className={
                            subteam.attributes.flaggedForDeletion ? 'line-through opacity-50' : ''
                          }
                        >
                        ...
                        <div className="ml-auto">
                          ...
                          >
                            {subteam.attributes.flaggedForDeletion ? (
                              <BanIcon size={15} />
                            ) : (
                              <Trash2Icon size={15} />
                            )}
                ...
                <SidebarMenuButton
                  className="cursor-pointer h-8"
                  onClick={() => setSubteamCreateOpen(true)}
                  asChild
                >
                  <a>
                    <SquarePlusIcon />
              ...
              <div className="flex flex-col flex-grow-1">
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 ml-[-5px] cursor-pointer"
                ...
                <div className="flex flex-col mt-2">
                  {Object.keys(bindleDefinitions[sharedResource]).length < 1 ? (
                    ...
                    <p className="text-muted-foreground text-sm">
                      {normalizeClientName(sharedResource)}
                    </p>
                  )}
...
const TeamSettingsDialog = (props: {
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.openChanged}>
      <DialogContent
        onOpenAutoFocus={(e) => e.preventDefault()}
        className="min-w-lg min-h-[60%] p-0 select-none"
      >
          ...
          <div className="flex flex-col mt-4">
            ...
            <div className="flex border-1 p-2 rounded-md mt-2 items-center">
              <div className="flex flex-col text-sm flex-grow-1">
                ...
                <p className="text-muted-foreground text-sm">
                  Configure your team's name and description
                </p>
              </div>
                ...
                <div key={clientName} className="flex flex-col mt-4">
                  ...
                  {Object.keys(props.settingDefinitions[clientName]).map((settingKey) => {
                    ...
                    return (
                      <div
                        key={settingKey}
                        className="flex border-1 p-2 rounded-md mt-2 items-center"
                      >
                        <div className="flex flex-col text-sm flex-grow-1">
                        ...
                        <Switch
                          checked={isChecked}
                          onCheckedChange={() => toggleSetting(clientName, settingKey, isChecked)}
                        />
                      </div>
                    )
                  })}
    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to remove <strong>{props.user?.name}</strong> from the{' '}
            <strong>{props.teamName}</strong> team? This will revoke their access to team resources
            immediately.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            This action cannot be undone and will remove all members from this subteam.
            Additionally, all subteam members will lose access to inherited team resources
            immideately.
          </DialogDescription>
        </DialogHeader>
