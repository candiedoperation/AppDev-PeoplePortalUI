...
const InfoItem = ({
  ...
  <div
    className={cn(
      'flex flex-col gap-1 p-3 rounded-lg border bg-card text-card-foreground shadow-sm',
      className
    )}
  >
    ...
    const fetchData = async () => {
      try {
        ...
        const teamsRes = await fetch(
          `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/people/${userData.username}/memberof`,
          {
            credentials: 'include',
          }
        )
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left gap-3">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-1.5 mt-1">
              <Badge
                ...
                className={cn(
                  'px-2 py-0.5 text-xs',
                  user.active ? 'bg-emerald-600 hover:bg-emerald-700' : ''
                )}
              >
              ...
              {user.is_superuser && (
                <Badge
                  variant="secondary"
                  className="gap-1 text-xs border-purple-500/30 text-purple-600 bg-purple-500/10 hover:bg-purple-500/20"
                >
              ...
              {attributes?.alumniAccount && (
                <Badge
                  variant="secondary"
                  className="gap-1 text-xs border-amber-500/30 text-amber-600 bg-amber-500/10 hover:bg-amber-500/20"
                >
          ...
          <div className="flex flex-col gap-2 text-sm text-muted-foreground border-t pt-4">
            ...
            {attributes?.phoneNumber && (
              <div className="flex items-center gap-2">
                ...
                <a
                  href={`tel:${attributes.phoneNumber}`}
                  className="hover:text-primary hover:underline truncate transition-colors"
                  title={attributes.phoneNumber}
                >
                  {attributes.phoneNumber}
                </a>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5">
            ...
            {attributes?.expectedGrad && (
              <InfoItem
                icon={Calendar}
                label="Class of"
                value={format(new Date(attributes.expectedGrad), 'yyyy')}
              />
            )}
            {user.memberSince && (
              <InfoItem
                icon={MapPin}
                label="Joined"
                value={format(new Date(user.memberSince), 'MMM yyyy')}
              />
            )}
            {user.last_login && (
              <InfoItem
                icon={Clock}
                label="Last Seen"
                value={formatDistanceToNow(new Date(user.last_login)) + ' ago'}
              />
            )}
            {user.type && user.type !== 'internal' && (
              <InfoItem
                icon={Tag}
                label="Account Type"
                value={<span className="capitalize">{user.type}</span>}
              />
            )}
          </div>
        </section>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <div className="flex items-center justify-between gap-2">
                        <CardTitle
                          ...
                          title={teamInfo?.friendlyName || teamPk}
                        >
                        ...
                        {teamInfo?.teamType && (
                          <Badge
                            variant="secondary"
                            className="text-[9px] h-4 px-1 rounded font-normal text-muted-foreground shrink-0"
                          >
                            {teamInfo.teamType}
                          </Badge>
                        )}
