...
export const DashboardPeopleList = () => {
  ...
  const columns: ColumnDef<UserInformationBrief>[] = [
        ...
        return (
          <div className="flex items-center">
            ...
            <div className="flex flex-col ml-3">
              ...
              {lastName && (
                <span className="text-xs text-muted-foreground uppercase">{lastName}</span>
              )}
            </div>
          </div>
        )
    ...
    },
    {
      ...
      header: 'Alias',
      cell: ({ row }) => {
        return <span className="font-medium text-sm text-foreground">{row.original.username}</span>
      },
    },
    {
      accessorKey: 'email',
  ...
  const refreshList = () => {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/org/people?${params.toString()}`)
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(userlistResponse.message || 'Failed to fetch')
  ...
  return (
    <div className="flex flex-col w-full h-full min-h-0">
      ...
      <div
        className={`flex-1 min-h-0 overflow-auto rounded-md border ${isLoading ? 'opacity-50' : ''}`}
      >
        <Table>
          <TableHeader>
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  onClick={() => {
                    navigate(`/org/people/${row.original.pk}`)
                  }}
