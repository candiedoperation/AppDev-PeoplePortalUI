...
export const OrgChartVisualization = () => {
  ...
  const renderCustomNode = useCallback(
    ({ nodeDatum, toggleNode, addChildren }: CustomNodeElementProps) => {
      ...
      const getHeaderStyle = () => {
        switch (node.type) {
          case 'ROOT':
            return 'bg-slate-900 border-slate-900 text-slate-50'
          case 'ROOT_MEMBER':
            return 'bg-blue-600 border-blue-600 text-white'
          case 'DIVISION':
            return 'bg-orange-100 border-orange-200 text-orange-700'
          case 'PERSON':
            return 'bg-purple-600 border-purple-600 text-white' // User asked for dark color
          default:
      ...
      const NodeCard = ({
        ...
        const needsLoad = depth >= 2 ? !hasVerticalChildren && node.hasChildren : needsLazyLoad
        ...
        return (
          ...
          >
            <div className="flex flex-col items-center">
              <Card
                className="w-full gap-2 flex flex-col overflow-hidden border shadow-sm hover:shadow-md transition-shadow"
        ...
        return (
          <g>
            ...
            {node.siblings?.map((sibling, index) => {
              const offset =
                CARD_WIDTH / 2 + 20 + ++index * (CARD_WIDTH + SIBLING_GAP) - CARD_WIDTH / 2
      ...
      return (
        <g>
          ...
          {node.siblings?.map((sibling, index) => {
            const offset =
              CARD_WIDTH / 2 + 20 + ++index * (CARD_WIDTH + SIBLING_GAP) - CARD_WIDTH / 2 // Calculate absolute X offset from center
  ...
  const pathClassFunc = useCallback((linkData: any) => {
    ...
    const targetDepth =
      linkData.target?.data?.__rd3t?.depth ??
      linkData.target?.__rd3t?.depth ??
      linkData.target?.depth ??
      0
      ...
      <div className="flex flex-col h-full items-center justify-center bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] dark:bg-[radial-gradient(rgba(200,200,200,0.1)_1px,transparent_1px)] [background-size:16px_16px]">
        <div className="flex flex-col items-center gap-2 p-6 rounded-lg border bg-card text-card-foreground shadow-sm max-w-md text-center">
          ...
          <p className="text-sm text-muted-foreground">
            The Executive Board could not be loaded. This usually means the "ExecutiveBoardMembers"
            team is empty or missing.
          </p>
        </div>
      </div>
    )
