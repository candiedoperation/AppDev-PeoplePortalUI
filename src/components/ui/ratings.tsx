...
const Ratings = ({ ...props }: RatingsProps) => {
  ...
  return (
    <div className={cn('flex items-center gap-0.5')} onMouseLeave={handleMouseLeave} {...props}>
      {[...Array(totalStars)].map((_, i) => {
        ...
        const isSolid = isHovering ? starValue <= hoverValue && starValue <= value : isFull

        const isPreview = isHovering ? starValue <= hoverValue && starValue > value : false
        ...
        return React.cloneElement(Icon as React.ReactElement<any>, {
          key: i,
          size,
          className: cn(fillClass, variantClass, asInput || onValueChange ? 'cursor-pointer' : ''),
...
const PartialStar = ({ ...props }: PartialStarProps) => {
  const { fillPercentage, size, className, Icon, asInput, onValueChange, onMouseEnter } = props
