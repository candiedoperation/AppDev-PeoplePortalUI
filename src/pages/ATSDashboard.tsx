...
const PERSONAL_INFO_FIELDS: PersonalInfoField[] = [
  ...
  { id: 'resumeUrl', label: 'Resume (Upload PDF Only)', type: 'file', required: true },
  {
    ...
    type: 'select',
    options: ['Yes 🥳', "No, I don't want to because I am lame ☹️", "I don't have Instagram"],
    required: true,
  },
  {
    id: 'whyAppDev',
  ...
  const handleSessionUpdate = (data: OTPSessionResponse) => {
    ...
    if (location.pathname.startsWith('/apply/') && !location.pathname.endsWith('/applications')) {
      ...
      if (
        possibleTeamId &&
        data.applications &&
        data.applications.some((app) => app.teamPk === possibleTeamId)
      ) {
      ...
      <header className="fixed top-0 left-0 right-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="flex h-16 items-center px-6 gap-4 max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="flex flex-col">Recruitment</div>
          </div>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-40" align="end" forceMount>
                  <DropdownMenuItem
                    ...
                    className="cursor-pointer py-2.5 text-destructive focus:text-destructive focus:bg-destructive/5"
                  >
          ...
          <Route
            ...
            element={
              <ATSApplyPage
                ...
                fullName={fullName}
                onRequestLogin={() => setTriggerLogin(true)}
              />
            }
          />
        </Routes>
...
export const ATSApplicationsList = ({
  applications,
  profile,
  fullName,
  email,
  ...
  return (
    <div className="flex flex-col gap-10 max-w-5xl mx-auto w-full px-4 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col gap-6">
        <Button
          className="max-w-max h-8 text-muted-foreground hover:text-foreground"
          onClick={() => navigate('../')}
          variant="ghost"
          size="sm"
        >
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-muted-foreground">
            Manage your applicant profile and track your active applications.
          </p>
        </div>
      </div>
        ...
        <CardHeader className="bg-muted/30 border-b pb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <CardDescription>
                This information is shared across all your applications.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            ...
            {PERSONAL_INFO_FIELDS.map((field) => {
              ...
              return (
                <div
                  key={field.id}
                  className="md:col-span-2 space-y-3 p-5 rounded-2xl border border-border/40 bg-card"
                >
                  <Label className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground flex items-center gap-2">
                    {field.id === 'linkedinUrl' && (
                      <LinkedinIcon className="h-3 w-3 text-primary/60" />
                    )}
                      ...
                      <a
                        href={
                          value.startsWith('http')
                            ? value
                            : `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/resume/download?key=${encodeURIComponent(value as string)}`
                        }
                        target="_blank"
                        rel="noopener noreferrer"
        ...
        {applications.length === 0 ? (
          ...
          <div className="grid gap-6">
            {applications.map((app) => (
              <Card
                key={app._id}
                className="border-border/50 shadow-sm hover:shadow-md transition-all overflow-hidden"
              >
                                ...
                                <div className="space-y-2">
                                  {app.rolePreferences?.map((pref, idx) => (
                                    <div key={idx} className="flex items-center gap-2 text-sm">
                                      <Badge
                                        variant="outline"
                                        className="text-[10px] font-bold h-5 w-5 flex items-center justify-center rounded-full p-0 shrink-0"
                                      >
                                        {idx + 1}
                                      </Badge>
...
const SortableRoleItem = ({ id, onRemove }: { id: string; onRemove: (id: string) => void }) => {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`flex items-center justify-between p-3 bg-secondary/30 border border-border/50 rounded-lg group hover:bg-secondary/50 transition-colors ${isDragging ? 'opacity-50 ring-2 ring-primary' : ''} touch-none`}
    >
        ...
        <div className="grid gap-2">
          {availableRoles.length === 0 ? (
            ...
            availableRoles.map((role) => (
              <div
                key={role}
                className="flex items-center justify-between p-3 bg-card border border-border/50 rounded-lg hover:border-primary/50 transition-colors group cursor-pointer"
                onClick={() => addRole(role)}
              >
                <span className="font-bold text-sm">{role}</span>
                <Button size="icon" variant="ghost" className="h-7 w-7 text-muted-foreground/60">
        ...
        <div className="bg-muted/10 rounded-xl border border-border/50 p-4 min-h-[200px]">
          {selectedRoles.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-4 text-muted-foreground opacity-60 gap-2">
              ...
              <p className="text-xs">
                Select roles from the left to add them here.
                <br />
                <span className="opacity-70 mt-1 block">
                  Your first choice is the most important!
                </span>
              </p>
            </div>
          ) : (
            ...
            >
              <SortableContext items={selectedRoles} strategy={verticalListSortingStrategy}>
                <div className="space-y-2">
...
const ATSApplyPage = ({
  ...
  React.useEffect(() => {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/openteams/${params.teamId}`, {
      credentials: 'include',
    })
      .then(async (res) => {
        if (!res.ok) {
  ...
  const handleApply = async () => {
    ...
    for (const field of PERSONAL_INFO_FIELDS) {
      ...
      if ((field.id === 'linkedinUrl' || field.id === 'githubUrl') && profile[field.id]?.trim()) {
        try {
          ...
          return toast.error(
            `Please enter a valid URL for ${field.id === 'linkedinUrl' ? 'LinkedIn' : 'GitHub'}.`
          )
        }
      }
    }
    ...
    if (teamData) {
      ...
      if (!responses[teamQuestionKey]) {
        return toast.error(
          `Please explain why you are interested in ${teamData.teamInfo.friendlyName}.`
        )
      }
  ...
  const handleResumeUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(
        `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/resume/upload-url?fileName=${encodeURIComponent(file.name)}&contentType=${encodeURIComponent(file.type)}`,
        {
          credentials: 'include',
        }
      )
      ...
      <div className="flex flex-col gap-6 py-6 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="text-4xl font-extrabold tracking-tight text-foreground">
            {teamData?.teamInfo.friendlyName}
          </h1>
          <div className="flex items-center gap-2">
            ...
            <CardHeader className="pt-0 flex bg-muted/40" style={{ padding: '15px' }}>
              <CardTitle className="pt-0 text-sm font-bold flex items-center gap-2.5 text-foreground/80">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-xs text-muted-foreground">
                    We'd love to know a bit about you!
                  </p>
                </div>
              </CardTitle>
              <CardDescription className="text-xs"></CardDescription>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="flex items-center">
                  <Label
                    htmlFor="resumeUrl"
                    className="text-sm font-bold flex items-center gap-1.5 text-foreground/80"
                  >
                    Resume (PDF)
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {profile.resumeUrl && (
                      <Button
                        ...
                        size="sm"
                        asChild
                        className="h-9 px-0 text-primary font-bold"
                      >
                        <a
                          href={
                            profile.resumeUrl.startsWith('http')
                              ? profile.resumeUrl
                              : `${PEOPLEPORTAL_SERVER_ENDPOINT}/api/ats/resume/download?key=${encodeURIComponent(profile.resumeUrl)}`
                          }
                          target="_blank"
                          rel="noopener noreferrer"
                        ...
                        {field.id === 'whyAppDev' && (
                          <div className="flex justify-between items-center px-1">
                            <span className="text-[10px] text-muted-foreground italic">
                              Tell us what motivates you to join.
                            </span>
                            <Badge
                              variant="outline"
                              className={`text-[10px] tabular-nums font-bold border-none bg-muted/30 ${(fieldValue.toString().split(/\s+/).filter(Boolean).length || 0) > 200 ? 'text-destructive' : 'text-muted-foreground'}`}
                            >
                              {fieldValue.toString().split(/\s+/).filter(Boolean).length || 0} / 200
                              words
                            </Badge>
                          </div>
                        )}
              ...
              <CardHeader className="pt-0 flex bg-muted/40" style={{ padding: '15px' }}>
                <CardTitle className="pt-0 text-sm font-bold flex items-center gap-2.5 text-foreground/80">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Share your skills and expectations
                    </p>
                  </div>
                </CardTitle>
                <CardDescription className="text-xs"></CardDescription>
              ...
              <CardContent className="flex flex-col gap-4 pt-0">
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    value={
                      responses[`Why are you interested in ${teamData?.teamInfo.friendlyName}?`] ??
                      ''
                    }
                    onChange={(e) =>
                      setResponses((prev) => ({
                ...
                {teamData &&
                  selectedRoles.map((role) => {
                    const subteamPk = roleToSubteamMap.get(role)!
                    const subteam = teamData.recruitingSubteams.find(
                      (s) => s.subteamPk === subteamPk
                    )
  ...
  return (
    <div className="flex flex-col max-w-5xl mx-auto w-full pb-12 px-4 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col gap-2 mt-8">
        ...
        <p className="text-muted-foreground">
          Join one of our world-class teams and build something amazing.
        </p>
      </div>

      <Accordion type="single" collapsible className="w-full space-y-4">
        {teams.map((team) => {
          const totalRoles = team.recruitingSubteamPks.reduce(
            (acc, pk) => acc + (team.subteamInfo[pk]?.recruitmentInfo?.roles.length || 0),
            0
          )
                ...
                <div className="flex items-center gap-4 w-full text-left">
                  ...
                  <div className="flex flex-col items-start min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-lg font-bold">{team.teamInfo.friendlyName}</span>
                      {hasApplied && (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pt-2">
                  <div className="flex-grow ml-[3.5rem]">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {team.recruitingSubteamPks
                        .flatMap((pk) => team.subteamInfo[pk]?.recruitmentInfo?.roles || [])
                        .map((role, idx) => (
                  ...
                  <Button
                    ...
                    onClick={() =>
                      hasApplied ? navigate(`/apply/applications`) : navigate(`./${team.teamPk}`)
                    }
                  >
...
const AccountLoginAndVerifyDialog = ({
  ...
  onSuccess: (data: OTPSessionResponse) => void
  fullName?: string
  open?: boolean
  ...
  const handleLogin = () => {
    ...
    fetch(`${PEOPLEPORTAL_SERVER_ENDPOINT}/api/auth/otpinit`, {
      ...
      .then(async (res) => {
        if (res.ok) setOtpPageVisible(true)
        else {
      ...
      <AlertDialogContent>
        ...
        <div className={`flex flex-col gap-2 ${otpPageVisible ? 'hidden' : ''}`}>
          ...
          <Input
            ...
            placeholder="Ex. atheesh@terpmail.umd.edu"
          />
          ...
          <Button
            ...
            disabled={
              isLoading ||
              !(
                (schoolEmail.endsWith('@umd.edu') || schoolEmail.endsWith('@terpmail.umd.edu')) &&
        ...
        <div className={`flex flex-col gap-2 ${!otpPageVisible ? 'hidden' : ''}`}>
          ...
          <p className="text-xs">
            Please enter the Verification Code sent to <b>{schoolEmail}</b>
          </p>
          <Button onClick={handleOtpVerify} className="mt-2" disabled={otp.length != 6}>
            <Loader2Icon className={`animate-spin ${!isLoading ? 'hidden' : ''}`} />
            Verify and Continue
          </Button>
        </div>
