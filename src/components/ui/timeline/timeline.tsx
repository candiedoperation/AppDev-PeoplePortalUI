...
import type { TimelineColor, TimelineStatus, TimelineSize } from './types'
  ...
  ({ className, iconsize, size, children, ...props }, ref) => {
    ...
    return (
      <ol
        ref={ref}
        aria-label="Timeline"
        className={cn(timelineVariants({ size }), 'relative w-full max-w-2xl mx-auto', className)}
        {...props}
      >
...
const TimelineItem = React.forwardRef<HTMLLIElement, TimelineItemProps>(
  (
    ...
    const commonClassName = cn(
      'relative w-full mb-0 last:mb-0', // Removed mb-8 to let connector bridge the gap
      className
    )
        ...
        >
          <div className="flex gap-4 items-start px-4">
            <div className="flex flex-col items-center justify-start gap-y-2">
              <div className="relative flex h-8 w-8 animate-pulse items-center justify-center rounded-full bg-muted ">
    ...
    if (error) {
      return (
        ...
        >
          <div className="flex gap-4 items-start px-4">
            <div className="flex flex-col items-center justify-start gap-y-2">
              <div className="relative flex h-8 w-8 items-center justify-center rounded-full bg-destructive/20 ">
    ...
    const content = (
      ...
      >
        ...
        <div className="flex flex-col items-center relative min-h-[5rem]">
          {' '}
          {/* Added min-height */}
          ...
          {showConnector && (
            <div className="absolute top-8 bottom-[-1rem] w-0.5 bg-border -z-0" />
            // Using absolute positioning to stretch from icon to next item
            // top-8 pushes it below the icon (depending on icon size)
            // bottom-[-1rem] extends it into the next item's space
          )}
        </div>

        {/* Content */}
        <TimelineContent>
          <TimelineTime className="block text-[10px]">{date}</TimelineTime>
          <TimelineHeader>
            <TimelineTitle className="leading-1">{title}</TimelineTitle>
          </TimelineHeader>
          <TimelineDescription className="text-[14px]">{description}</TimelineDescription>
        </TimelineContent>
      </div>
    )
...
const TimelineIcon = ({
  ...
  const colorClassName = colorClasses[color] || color
